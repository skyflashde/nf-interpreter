...
HRESULT CLR_GFX_Bitmap::CreateInstance( CLR_RT_HeapBlock& ref, const CLR_GFX_BitmapDescription& bm )
{
    ...
    if (size + sizeof (CLR_RT_HeapBlock_BinaryBlob) < (sizeof(CLR_RT_HeapBlock) * CLR_RT_HeapBlock::HB_MaxSize / 2))
    {
        ...
        NANOCLR_CHECK_HRESULT(CLR_RT_HeapBlock_BinaryBlob::CreateInstance( ref, size, NULL, CLR_GFX_Bitmap::RelocationHandler, CLR_RT_HeapBlock::HB_CompactOnFailure ));
...
HRESULT CLR_GFX_Bitmap::CreateInstance( CLR_RT_HeapBlock& ref, const CLR_UINT8* data, CLR_UINT32 size, CLR_RT_Assembly* assm )
{
    ...
    if((bm->m_flags & CLR_GFX_BitmapDescription::c_Compressed) != 0)
    {
        ...
        NANOCLR_CHECK_HRESULT(CLR_GFX_Bitmap::GetInstanceFromHeapBlock (refUncompressed, bitmap));
    ...
    // Since we're now bpp-agnostic we'll always do the convert
    {
        ...
        NANOCLR_CHECK_HRESULT(CLR_GFX_Bitmap::GetInstanceFromHeapBlock (ref, bitmapNative));
...
void CLR_GFX_Bitmap::Screen_Flush(CLR_GFX_Bitmap& bitmap, CLR_UINT16 x, CLR_UINT16 y, CLR_UINT16 width, CLR_UINT16 height)
{

...
void Graphics_DrawRoundedRectangle(const PAL_GFX_Bitmap& bitmap, const GFX_Pen& pen, const GFX_Rect& rectangle,
    int radiusX, int radiusY)
{
    Graphics_Driver::DrawRoundedRectangle(bitmap, pen,  rectangle, radiusX, radiusY);
}
...
CLR_UINT32 Graphics_Driver::GetPixel(const PAL_GFX_Bitmap& bitmap, int x, int y)
{
    return ConvertNativeToColor(GetPixelNative(bitmap, x, y));
}
void Graphics_Driver::SetPixel(const PAL_GFX_Bitmap& bitmap, int x, int y, CLR_UINT32 color)
{
...
void Graphics_Driver::DrawRoundedRectangle(const PAL_GFX_Bitmap& bitmap, const GFX_Pen& pen, const GFX_Rect& rectangle,
    int radiusX, int radiusY)
{
    GFX_Pen nativePen = ConvertPenToNative(pen);
    DrawRoundedRectangleNative(bitmap, nativePen,  rectangle, radiusX, radiusY);
}
...
int Graphics_Driver::GetSize(int width, int height)
{
    if (width < 32768 && height < 32768)
    {
        ...
        // 0x7FFF * 0x7FFF * 4 = 0xFFFC0004, which would fit in CLR_UINT32. So no need to do the 64-bit
        // math.
...
void Graphics_Driver::SetPixelNative(const PAL_GFX_Bitmap& bitmap, int x, int y, CLR_UINT32 color)
{
    if (IsPixelVisible(bitmap.clipping, x, y))
    {
        CLR_UINT32  mask, shift;
        ...
        if (brush.gradientStartColor == brush.gradientEndColor || (gradientDeltaX == 0 && gradientDeltaY == 0))
        {
            ...
            if (insetWidth > 0 && insetHeight > 0)
            {
                ...
                else // if (opacity != PAL_GFX_Bitmap::c_OpacityOpaque)
                {
                    ...
                    CLR_UINT16 interpolated = NativeColorInterpolate(fillColor, lastPixel, opacity);
                    for (int row = 0; row < insetHeight; row++, curRow += stride)
                    {
                        curPixel = curRow;
                        for (int col = 0; col < insetWidth; col++, curPixel++)
...
void Graphics_Driver::DrawRoundedRectangleNative(const PAL_GFX_Bitmap& bitmap, GFX_Pen& pen, const GFX_Rect& rectangle, int radiusX, int radiusY)
{
...
void Graphics_Driver::RotateImage(CLR_INT16 degree, const PAL_GFX_Bitmap& dst, const GFX_Rect& dstRect, const PAL_GFX_Bitmap& src, const GFX_Rect& srcRect, CLR_UINT16 opacity)
{
    float  cos, sin;
...
void Graphics_Driver::DrawImage(const PAL_GFX_Bitmap& bitmapDst, const GFX_Rect& dst, const PAL_GFX_Bitmap& bitmapSrc, const GFX_Rect& src, CLR_UINT16 opacity)
{
    ...
    else // StretchImage
    {
        ...
        for (int y = 0; y < croppedHeight; y++, pd += strideDst)
        {
            ...
            CLR_UINT16 color = *ps;
            CLR_UINT32 nativeTransparentColor = ConvertColorToNative(bitmapSrc.transparentColor);
...
CLR_UINT32 Graphics_Driver::NativeColorInterpolate(CLR_UINT32 colorTo, CLR_UINT32 colorFrom, CLR_UINT16 scalar)
{
...
void Graphics_Driver::SetPixelsHelper(const PAL_GFX_Bitmap& bitmap, const GFX_Rect& rect, CLR_UINT32 config, GFX_SetPixelsCallback callback, void* param)
{
    int x = rect.left;
    int y = rect.top;
